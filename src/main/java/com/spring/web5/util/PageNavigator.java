...
public class PageNavigator {
	...
	private int pagePerGroup;		// 그룹 당 페이지 수
	private int currentPage;		// 현제 페이지 (최근 글이 1 부터 시작)
	 ...
	 * 생성자
	 */
	public PageNavigator(int countPerPage, int pagePerGroup, int currentPage, int totlaRecordsCount) {
		// 페이지 당 글 수, 그룹 당 페이지 수, 현재 페이지, 전체 글 수를 전달받음
		this.countPerPage = countPerPage;
		this.pagePerGroup = pagePerGroup;
		this.totalRecordsCount = totlaRecordsCount;
		...
		// 전달된 현제 페이지가 마지막 페이지보다 크면 현재 페이지를 마지막 페이지로 지정
